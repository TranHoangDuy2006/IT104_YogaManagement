...
function ClassesManagementPage() {
  ...
  const { currentPage, totalPages, currentItems, handlePageChange } = usePagination(courses, itemsPerPage);
            ...
            <tbody>
              {currentItems.map((course, idx) => (
                <tr key={course.id} className="border-t border-gray-200 hover:bg-gray-50 transition">
                  <td className="px-3 py-4 text-[17px] text-center font-semibold">{(currentPage - 1) * itemsPerPage + idx + 1}</td>
